<div class="p-6 mb-5 bg-white shadow-lg rounded-lg border border-gray-200">
    <div class="md:flex justify-between items-start">
        <a href="{{ route('events.show', $event->id) }}" class="flex-grow">
            ...
            @endif
            <h2 class="block mt-1 text-lg md:text-xl leading-tight font-bold text-transparent bg-clip-text bg-gradient-to-r from-dark-green to-mint hover:underline">{{ Str::limit($event->name , $limit = 55, $end = '...') }}</h2>
        ...
        @if (isset($isFavoriteView) && $isFavoriteView)
            <form method="POST" action="{{ route('favorites.remove', $event->id) }}" class="mt-4 md:mt-0">
                @csrf
                @method('DELETE')
                <x-events.button-gradient type="submit">Supprimer</x-events.button-gradient>
            </form>
        @endif
